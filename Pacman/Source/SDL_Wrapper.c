...
void SDLWrapper_Init(const char* windowName, GraphicsState* state)
{
    ...
    state->window = SDL_CreateWindow(
        windowName,
        state->windowTransform.position.x,
        state->windowTransform.position.y,
        state->windowTransform.size.x,
        state->windowTransform.size.y,
        SDL_WINDOW_SHOWN
    );
    ...
    PAC_LOG("Successfully initialized SDL");
    PAC_LOG("Successfully initialized SDL image");
    PAC_LOG("Successfully initialized SDL TTF");
}
